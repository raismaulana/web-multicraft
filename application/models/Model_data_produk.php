...
class Model_data_produk extends CI_Model {
	...
	public function tabel($datatabel){
		...
		foreach($datatabel as $row){
			$this->table->add_row(
				...
				$row->id_kategori,
				$row->nama_kategori,
				anchor('dashboard/edit'.$row->id_kategori, 'Edit', array('class'=>'btn btn-primary')),
				anchor('dashboard/hapus/'.$row->id_kategori, 'Delete', array('class'=>'btn btn-danger'))
			);
		}
